...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.3 | 2026-06-04 | 归档 2026-06-04 上午两个 `feat` 提交的产品变更：V1 原型流程重排、版本边界调整、就诊记录增强与回显闭环 |
...
## 10. v1.3 · 2026-06-04 上午 feat 提交内容归档

本节归档 2026-06-04 上午两个 commit message 含 `feat` 的提交内容，并转写为 PRD 口径。两个提交分别是：

| Commit | 时间 | 标题 | PRD 影响 |
|---|---:|---|---|
| `a6d3bd3` | 08:14 | `feat: update v1.1 prototype flows` | 重排 V1/V2/V3 原型边界，补齐现场上报照片、诊断草稿、单通道数据流和版本流程页表达 |
| `c277d44` | 11:57 | `feat: refine v1 visit record flow` | 收紧 V1 就诊记录闭环，新增手填补录、附件回显、多次更新、出院后回看，并将独立蛇伤判定整体移至 V3 |

### 10.1 最终产品口径

- **V1 只跑最小急救接诊闭环**：患者小程序 → 医生 PC → 内网最小导入；不经过医生 APP、转诊、MDT、共享、患者 360、随访陪护等 V2 能力。
- **V1 蛇种判定不再使用独立规则表单**：医生在「就诊记录」内直接填写最终蛇种、血清用量和处置；独立「蛇伤判定（规则表单）」作为 V3 规则复盘、图像识别、Agent 初诊和治疗路径增强入口。
- **V1 就诊记录是治疗记录单**：首次保存形成最小就诊档案并推进到「治疗中」；治疗中可多次进入同一页更新；出院后原型仍允许再次编辑以便演示纠错，状态保持「已出院」。
- **V1 缺失医疗数据手填补录**：诊断说明、用药、检验、影像/附件、生命体征均由医生手填文本补录；结构化 LIS/PACS/HIS 自动回填放到 V2。
- **V1 出院病例仍可在医生工作台回看**：救助工作台列表包含已出院病例，避免确认出院后医生端看不到患者。
- **版本入口合并为「模块版本流程」**：用一个页面呈现模块范围、版本边界、主流程和演示起点，替代早期首页/演示说明的重复表达。

### 10.2 患者端变更

- 患者端健康陪护相关入口整体标记为 V2：我的随访计划、复查详情、用药打卡、陪护对话、打卡日历、随访问卷、知识中心、我的医生、我的健康等不纳入 V1 主线。
- 患者「现场信息上报」支持现场照片占位图，医生端可在实时视图中查看患者上传的蛇体/伤口线索。
- 患者「等待页」在医生填写就诊记录后展示就诊摘要，并可展开查看检验、影像、生命体征补录和附件缩略图。
- 患者「我的就诊」展示最新就诊记录内容，包含诊断、血清和补录状态。

### 10.3 医生 PC 端变更

- 「救助工作台」列表改为包含已出院病例，操作按钮在已出院时显示「回看」。
- 「患者实时视图」新增就诊记录流转区：
  - 展示蛇种判定与就诊记录归档状态。
  - 展示诊断、用药、检验、影像四类补录完成状态。
  - 已有就诊记录时完整回显治疗记录和附件缩略图。
- 「就诊记录」页扩展为 V1 手填补录入口：
  - 新增诊断说明/鉴别依据、用药记录、检验摘要、影像/附件摘要、生命体征摘要。
  - 新增图片附件区，默认使用静态占位图生成伤口照片、检验单、影像附件 3 类附件。
  - 所有新增字段提供默认值，医生可以直接保存或现场微调。
  - 再次进入页面时回填已保存内容，不被默认值覆盖。
- 「蛇伤判定（规则表单）」从 V1/V2 主流程移至 V3，仅作为规则判定复盘与智能化增强入口；V1 主流程不出现独立判定页。

### 10.4 医生 APP 与协同能力变更

- 医生 APP 整体移至 V2，包含收件箱、移动就诊记录、转诊详情、MDT 邀请、MDT 会诊室、共享给我的患者。
- 多中心协同整体移至 V2，包含转诊申请/跟踪、MDT 邀请/会诊室、共享患者/共享给我的患者、患者 360。
- 医生 APP「就诊记录」保留只读展示同源治疗记录与附件，用于 V2 移动查看；移动补录按钮仍作为 V2 占位。

### 10.5 数据与内网端变更

- 数据接入从「冷热双通道」改为单一接口/同步通道，不追求秒级时效。
- 内网端 V1 只保留「旧专病库导入 + 字段映射 + 去重 + 导入错误提示」的最小导入能力。
- 原「数据接入监控」路由保留但菜单隐藏，避免 V1 演示进入偏技术对接页面。
- 质疑工单详情增加「查阅原文/复核依据」和审计日志占位，作为 V3 数据质量复核能力。
- 数据流转大屏去掉冷热双通道切换，改为单通道展示和内外网边界水印。

### 10.6 数据模型与状态规则

- `FieldReport` 增加 `photoUrl`，用于患者现场照片占位与医生端回显。
- `RescueEvent` 增加 `diagnosisDraft`，用于保存 V3 判定草稿；V1 就诊记录可读取草稿来源时间，但不依赖独立判定流程。
- `VisitRecord` 扩展为完整治疗记录：
  - `diagnosisNote`
  - `medications`
  - `labSummary`
  - `imagingSummary`
  - `vitalSigns`
  - `attachments`
- `saveVisitRecord` 行为调整：
  - 首次保存：写入 `visitRecord` 并推进状态到「治疗中」。
  - 后续更新：覆盖同一事件的最新 `visitRecord`，不改变当前状态。
  - 出院后更新：允许保存，状态保持「已出院」。
- 新增 `workbenchEvents`：医生工作台读取全量事件，包括已出院事件；`activeEvents` 仍用于统计待处理事件。

### 10.7 V1 验收补充

- 未登录或已登录患者均可完成 SOS、定位、选医院、上报现场信息、等待进度和出示二维码。
- 现场照片能在患者上报页、医生实时视图和就诊记录附件中看到静态占位图。
- 首次填写就诊记录后状态进入「治疗中」。
- 治疗中再次进入就诊记录页，已填写内容和附件回填，不被默认值覆盖。
- 修改补录字段后保存，患者端等待页/我的就诊与医生实时视图展示最新内容。
- 确认出院后，医生工作台仍能看到该患者并进入回看。
- 独立蛇伤判定页在 V1 主流程中不作为必经步骤；V3 视图下可作为增强入口访问。
- 模块版本流程页能直接说明 V1/V2/V3 边界与演示入口，避免首页、版本规划、演示说明重复。
