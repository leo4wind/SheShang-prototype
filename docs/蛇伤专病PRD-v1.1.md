...
flowchart TD
    ...
    J --> K[蛇伤判定<br/>V1 规则表单 · 人工确认]
    K --> L[手填就诊记录<br/>蛇种/血清用量/处置]
    L --> M[状态：治疗中 → 已出院]
    M --> N[患者查看就诊历史]
...
4. **脱敏口径**：仅前端展示脱敏，后端不脱敏、保留复核能力。原型阶段不展开复杂隐私/审计边界。
5. **专病库是结构化数据唯一对外层**：消费侧（接诊/协同/随访/科研）统一从专病库读，不直连源系统。
...
| P204 | 蛇伤判定 | 救助 | V1 规则表单 / **V2 AI 占位** | V1 仅规则表单；AI 建议依赖 V3 知识库 |
...
> **蛇伤判定（P204）分级**：V1 仅「规则化蛇伤判定表单 + 医生人工确认」，去掉 AI；V2 加 AI 占位卡片并标「仅辅助、不自动诊断」；V3 接图像识别 + Agent 初诊 + 治疗路径。
...
| # | 用户故事 | 角色 | 验收要点 |
|---|---|---|---|
| 1 | 作为患者，我想一键 SOS 并自动定位，以便慌乱时也能快速发起求救 | 患者 | 点击大按钮即登记事件、定位失败可手动选点 |
...
| 7 | 作为接诊医生，我想用规则表单判定蛇种并手填就诊记录，以便形成最小档案 | 接诊医生 | V1 无 AI，纯规则表单 + 人工确认；记录可手填 |
| 8 | 作为患者，我想到院出示就诊二维码，以便前台核验并标记已到院 | 患者 | 扫码后事件状态变「已到院」 |
| 9 | 作为患者，我想查看自己的就诊历史，以便了解诊疗过程 | 患者 | 就诊历史列表 + 详情 |
...
| P204 蛇伤判定 | v2 | **v1（规则表单）+ v2（AI 占位）** | V1 去 AI、留规则表单 |
...
- **首页 / 版本规划页**：从「四条并列主线」改为「三层故事」——V1 急救闭环 / V2 协同随访 / V3 治理科研。
- **演示模式**：V1 主演示路径只串 3 终端（患者小程序 → 医生 PC → 内网最小导入），不经过医生 APP / 转诊 / MDT。

### 9.3 页面级改造

- **蛇伤判定（`doctor/DiagnosisPage.vue`）**：V1 视图隐藏 AI/图像识别区块，保留规则化判定表单；AI 占位区块加版本标签 V2 + 「仅辅助、不自动诊断」提示。
